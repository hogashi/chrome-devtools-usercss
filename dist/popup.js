...
var getHostnameSet = function getHostnameSet() {
  return JSON.parse(localStorage.getItem(HOSTNAME_SET) || '{}');
};

var addHostname = function addHostname(hostname, set) {
  set[hostname] = true;
  localStorage.setItem(HOSTNAME_SET, JSON.stringify(set));
};

var getStyleByHostname = function getStyleByHostname(hostname) {
  return localStorage.getItem(hostname) || '';
};

var addOrUpdateStyleAndHostname = function addOrUpdateStyleAndHostname(set, hostname, style) {
...
var rememberLasSelectedHostname = function rememberLasSelectedHostname(hostname) {
  return localStorage.setItem(LAST_SELECTED_HOST_NAME, hostname);
...
if (lastSelectedHostname && hostnameSet[lastSelectedHostname]) {
  document.querySelector("option[value=\"".concat(lastSelectedHostname, "\"]")).selected = true;
  onLoadButtonClick();
}
/******/ })()
;